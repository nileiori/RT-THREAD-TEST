...
* Input          : priorGrp:	 Priority Group	see "misc.h"
*									 addrOffset: Vector Table base offset field. This value must be a multiple of 0x200.
* Output         : None
* Return         : None
*******************************************************************************/
...
void SysTick_Handler(void)
{
	...
	if(count++ >= RT_TICK_PER_SECOND/2)//0.5秒更新1次时间
	{
		count = 0;
		ReadRtc();
...
void rt_hw_board_init(void)
{
	...
	/* 配置终端优先级组，app偏移量 */
	NVIC_Configuration(NVIC_PriorityGroup_0,0x4000);

	/* Configure the SysTick */
	SysTick_Config( SystemCoreClock / RT_TICK_PER_SECOND );//1000Hz

#if STM32_EXT_SRAM
	EXT_SRAM_Configuration();
#endif
#ifdef	RT_USING_CONSOLE	//管理串口
	rt_hw_usart_init();
	rt_console_set_device_special("uart5");
	rt_console_set_device(RT_CONSOLE_DEVICE_NAME);
#endif
#ifdef	CUSTOM_USING_TEMPLATE
	custom_template_init();
	extern void custom_test(void);
	custom_test();
#endif
#ifdef RT_USING_PIN	//管理pin脚
	stm32_hw_pin_init();
#endif
	//rt_hw_sdcard_init();
