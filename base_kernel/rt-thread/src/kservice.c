...
rt_device_t rt_console_set_device_special(const char *name)
{
    rt_device_t new, old;
    ...
    if (new != RT_NULL)
    {
        ...
        /* set new console device */
        rt_device_open(new, RT_DEVICE_OFLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_INT_TX);
        _console_device = new;
    }

    return old;
}
RTM_EXPORT(rt_console_set_device_special);
