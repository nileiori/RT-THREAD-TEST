...
//读串口数据
rt_size_t stm32_serial_read(struct rt_serial_device *serial,rt_off_t pos,void* buffer,rt_size_t size)
{
...
//写串口数据
rt_size_t stm32_serial_write(struct rt_serial_device *serial,rt_off_t pos,void* buffer,rt_size_t size)
{
		struct rt_device *dev;
...
{
		return stm32_serial_read(&serial1,pos,buffer,size);
}
//写串口1数据
rt_size_t stm32_serial1_write(rt_off_t pos,void* buffer,rt_size_t size)
{
		return stm32_serial_write(&serial1,pos,buffer,size);
}
...
{
		return stm32_serial_read(&serial5,pos,buffer,size);
}
//写串口5数据
rt_size_t stm32_serial5_write(rt_off_t pos,void* buffer,rt_size_t size)
{
		return stm32_serial_write(&serial5,pos,buffer,size);
}
